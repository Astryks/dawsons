...
- **EQ, compression & delay** — three more Ableton/Logic-style effects, all from-scratch DSP (a textbook biquad peaking filter using the standard "Audio EQ Cookbook" formulas, a classic feedforward envelope-follower compressor, and a feedback delay line), joining reverse/pitch/reverb in one unified `EffectChain` any clip-tools file goes through. 13 new Rust tests.
- **Timeline improvements** — layers are now proportionally sized by real duration, reorderable (move up/down) and removable from the UI, and a moving playhead tracks live playback position (polled via a new `transport_status` command that reports position + whether playback is still running, so the UI can tell a real pause from playback finishing naturally). Not yet the full draggable multi-region editor M6 calls for, but a real, functional upgrade from the static colored-lane summary.

58 Rust unit tests + 16 Python tests (including real end-to-end runs — Demucs inference, the full M5 pipeline, and clip classification, not mocked) passing. Everything above runs 100% locally — no server, no cloud cost, per the project's core constraint.
