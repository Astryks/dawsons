...
| **Custom chroma+template chord detector** (our own code, `chords.py`) | N/A (original code) | Chord recognition — built in-house specifically to avoid Chordino (GPL-2.0). Now covers 8 chord qualities (maj/min/dom7/maj7/min7/sus4/sus2/dim) with HPSS-based noise rejection (see Test results). |
...
- **DDSP** (Magenta, Apache-2.0) — real timbre-preserving voice-to-instrument (vs. today's "detect notes, replay through GM synth" approach). Bigger effort.
...
- **Chord detector accuracy** (the one place this project has done real before/after measurement): on a real 2-minute audio clip, the fix for percussion-induced over-segmentation cut chord segments from 335 (averaging 0.26s each — physically impossible for real music) down to 64 (averaging 2.8s each — a realistic chord-change rate). Verified with synthesized ground-truth chord fixtures built from publicly documented, uncopyrightable chord names, not real recordings.
