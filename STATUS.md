...
28 Rust unit tests passing. Everything above runs 100% locally — no server, no cloud cost, per the project's core constraint.

## In progress

- **M4 — Real song analysis (Demucs stem separation)**: installing PyTorch/Demucs in the sidecar now, wiring `POST /analyze` → job polling → Rust loads real stems as playable tracks.

## Not started yet

- M5 — Tempo/key/chord/section detection populating the full Scene Graph
